...
# 第 6.5 章　那為什麼板子不能自己訓練？

你會問這個問題，代表你已經把前面幾章串起來了：板子上有 NPU、有 128 MB 記憶體、
跑一次推論只要 0.12 ms——看起來什麼都有，為什麼還要把資料搬去 Colab？

這一章把它算清楚。答案有三層，而且**最直覺的那個「算力不夠」是錯的**。

## 先把算力算掉，因為它根本不是瓶頸

訓練一次要做多少運算？拿我們的猜拳模型來算：

```
推論一次          = 30.67 M MAC
訓練一次(一張圖)  = 推論 × 3      ← 前向 1 + 反向 2(算梯度、傳梯度)
                  = 92 M MAC

300 張 × 30 epochs = 9000 次
總量 = 92 M × 9000 = 828 G MAC = 1.66 T ops
```
...
| 拿什麼算 | 時間 |
|---|---|
| NPU 理論值 0.4 TOPS | **4 秒** |
| NPU 實際利用率 5.5% | **75 秒** |
| KM4 用軟體硬幹 @500 MHz | **~2 小時** |
| Colab 的 GPU | **30 秒** |

**四秒。** 就算打到 5.5% 的利用率也才一分多鐘。
**算力完全夠。** 所以「板子太慢所以不能訓練」這個說法是錯的。

真正的原因是下面這三個。

## 原因一：NPU 是單行道，它沒有反向的零件

第 6 章說過 NPU 是「400 個只會一招的工人」。那一招是**乘加**，而且是
**推論方向的**乘加。訓練需要的東西它一樣都沒有：

| 訓練需要 | NPU 有嗎 |
|---|---|
| 前向傳播（算出答案） | ✓ 這就是它唯一會的事 |
| **反向傳播**（算每個旋鈕該轉多少） | ✗ 沒有這種電路 |
| **權重可寫** | ✗ `.nb` 裡的權重是 uint8 **唯讀**，開機時搬進 DDR 就固定了 |
| **動態改變計算圖** | ✗ 圖在轉檔那一刻就編死了（第 7 章） |

最後一點最致命。回想 `.nb` 是什麼：**編譯完成的算圖**。
它不是「模型檔」，是「這 400 個工人第 1 拍做什麼、第 2 拍做什麼」的排程表。
訓練要一邊改權重一邊改計算過程——那等於要在執行中重新編譯。

> 用你熟的話講：**`.nb` 之於 NPU，像燒進 flash 的 `.bin` 之於 MCU。**
> 你不會期待一顆 MCU 在跑的時候自己改寫自己的 `.text` 段。

## 原因二：記憶體差一個數量級，而且差在你想不到的地方

直覺會說「權重才 833 KB，128 MB 綽綽有餘」。錯在沒算另外三樣：

| 訓練時記憶體要放什麼 | 大小 | 為什麼 |
|---|---|---|
| 權重（**fp32**，不是 uint8） | 833 KB | 梯度是很小的數，uint8 的解析度會把它整個吃掉 |
| 梯度 | 833 KB | 每個權重都要有一份 |
| Adam 的動量與變異數 | 1.67 MB | 每個權重要**再兩份** |
| **中間活化值** | **~100 MB** | ★ 就是這個 |
|  | | |
| NN 區總共有 | **36 MB** | |

**為什麼活化值這麼大？** 因為反向傳播要用到前向時每一層的輸出，
所以**每一層的 feature map 都得留著**，而且是整個 batch 一起留：

```
第一層輸出 96×96×16 × 4 bytes(fp32) = 590 KB  ← 一張圖
× batch 32                            = 18.9 MB ← 光第一層
再加上其他三層 conv 與 dense           ≈ 100 MB
```

推論不需要留，算完下一層就可以把上一層丟掉——**這就是為什麼推論
只要 36 MB，訓練卻要 100 MB 以上**。

把 batch 降到 1 可以擠進去，但那樣訓練會極不穩定，而且前面兩個原因還在。

## 原因三：板子上沒有 TensorFlow

第 5 章講過 TensorFlow 的角色 = **gcc**。

訓練 = 編譯。你不會在 MCU 上放一套 gcc，同樣的，
板子上也沒有那套「會微分、會算梯度、會做最佳化」的框架。
它只有一個執行期：`.nb` 進來、畫素進來、機率出去。
...
```
凍住   Conv → Conv → Conv → Conv → Dense64     ← 這段當成固定的「特徵抽取器」
只訓   Dense(64 → N)                           ← 只有這裡的權重會動
```

| | 全部重訓 | 只訓最後一層 |
|---|---|---|
| 要改的參數 | 208,292 | **數百個** |
| 中間活化值 | ~100 MB | **只要最後那層的輸入,幾 KB** |
| 要幾張圖 | 幾百張 | **每類 5~10 張** |
| 能不能在板子上做 | ✗ | **✓** |

而且前面那段**不用改權重**，所以可以照常用 NPU 跑（它只是在做推論），
最後那幾百個參數用 KM4 的軟體去調，一下就好。

**你已經看過這個東西了。** 手機的人臉解鎖就是這樣：
它沒有「為你重新訓練一個模型」，它是拿一個**通用**的人臉特徵模型
（AMB82 上就是 `mobilefacenet.nb`），把你的臉轉成一串數字存起來，
之後比對距離。錄入只要拍幾張、幾秒鐘——因為真正在「學」的只有最後那一小塊。
...
我們沒有做微調，是因為猜拳的四個類別是固定的——手勢不會因人而異到需要每台裝置各學一次。
但如果你要做的是「**認得我這個人的手勢習慣**」或「**讓使用者自訂一個新手勢**」，
那就正好是微調的題目，而且**可以在板子上做**。
