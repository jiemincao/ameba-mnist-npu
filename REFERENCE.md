...
> **這份是參考手冊，不是教材。** 給已經在做這件事的人查東西用：
> 分區表、指令、實測數字、踩過的坑。
>
> 第一次接觸請先看 [**COURSE.md**](COURSE.md)。
>
...
```mermaid
flowchart LR
  subgraph PC["💻 PC / Colab —— 離線,整個專案只做幾次"]
    direction TB
    A[訓練資料<br/>一堆照片 + 標籤] --> B[TensorFlow<br/>訓練]
    B --> C["模型檔<br/>.tflite"]
    C --> D["acuity 轉檔<br/>量化 + 編譯"]
    D --> E["<b>.nb</b><br/>NPU 算圖"]
  end

  subgraph BOARD["📷 AMB82 板子 —— 上線後,每一幀都在跑"]
    direction TB
    F[鏡頭 sensor] --> G[ISP<br/>曝光 / 白平衡]
    G --> H{{VOE<br/>影像協同處理器}}
    H -->|channel 0| I[JPEG 快照]
    H -->|channel 3| J[RGB 張量]
    J --> K[StreamIO]
    K --> L[["NPU"]]
    L --> M[每類一個機率]
    M --> N[KM4<br/>你的 sketch]
    I --> N
    N --> O[UART → PC 的 GUI]
  end

  E -.->|燒進 flash 的 nn 分區| L

  style E fill:#f9a825,stroke:#333,color:#000
  style L fill:#42a5f5,stroke:#333,color:#000
  style H fill:#66bb6a,stroke:#333,color:#000
```
...
搞清楚這張圖，你的問題就有位置了：
相機在右上、TensorFlow 在左上、NPU 在右下、`.nb` 在正中間。
...
| **SRAM** | 晶片內 ~512 KB（ITCM / DTCM / RAM_1 / RAM_2 / retention） | 程式的堆疊與熱資料 |
| **DDR** | **128 MB**，跟晶片同封裝 | 影像緩衝、模型權重 |
| **Flash** | 16 MB SPI NOR | 韌體 + 模型，斷電不會消失 |

> ⚠ **DDR 不是 SD 卡。** DDR 是焊死在模組上的工作記憶體，斷電就空了；
> SD 卡是可插拔的檔案儲存。兩者完全不同的東西。

**這顆晶片裡有三個處理器在各做各的。**
KM4 跑你的程式、VOE 管畫面、NPU 算模型。
你的 sketch 大部分時間只是在**協調**它們，不是在做事。

### 記憶體地圖（從 linker script 拿的，不是猜的）

`variants/ameba_amb82-mini/linker_scripts/gcc/amebapro2_ram_arduino.ld`：

```
DDR_SIZE = 128;   /* MB */
NN_SIZE  = 36;    /* MB */

0x00010000  ITCM_RAM         32 KB    指令 TCM,零等待
0x20000000  DTCM_RAM         16 KB    資料 TCM
0x20106300  RAM_1         106.5 KB
0x20120000  RAM_RETENTION   128 KB    深睡時保住的資料
0x20140000  RAM_2           222 KB
                          ─────────
                          ~512 KB     ← 晶片內 SRAM 總量

0x70000000  VOE               1 MB    ★ 相機獨佔,KM4 不要碰
0x70100000  DDR              91 MB    一般用途:影像緩衝、堆積
0x75C00000  NN               36 MB    ★ NPU 專用:權重 + 中間張量
                          ─────────
                           128 MB     ← DDR 總量
```

**為什麼 NPU 要獨立切 36 MB**：`.nb` 載入後權重要攤在連續記憶體裡，
中間張量（feature map）也要地方放。切開才不會跟一般的 `malloc` 打架。

### Flash 怎麼分（16 MB，這是你問的）

分區表是 **Realtek 的 `amebapro2_partitiontable.json`**，燒錄時會一起寫進去：

| 分區 | 起始 | 大小 | 放什麼 |
|---|---|---|---|
| `sysdata` / `fcsdata` | 0x007000 | 4 KB × 2 | 系統參數、工廠校正 |
| `boot_p` | 0x009000 | 156 KB | 主 bootloader |
| `boot_s` | 0x030000 | 156 KB | 備援 bootloader |
| **`fw1`** | 0x060000 | **4 MB** | **韌體：你的 sketch + WiFi + BT + 影音 + FreeRTOS** |
| `iq` | 0x460000 | 768 KB | ISP 的畫質調校參數（鏡頭專屬） |
| `fw2` | 0x520000 | 64 KB | OTA 的第二槽（非 OTA 版本只留殘根） |
| **`nn`** | 0x530000 | **10.56 MB** | **模型 `.nb`** |
| `mp` | 0xFC0000 | 4 KB | 量產測試 |

**⚠ WiFi 不是獨立分區。** WiFi、藍牙、TCP/IP、H.264 編碼器全部是**靜態函式庫**，
編譯時連進你的 `.bin`，一起躺在 `fw1` 那 4 MB 裡：

```
variants/common_libs/libwlan.a      20 MB   ← WiFi 驅動 + 韌體 blob
variants/common_libs/btgap.a       853 KB   ← 藍牙
variants/common_libs/libarduino.a   32 MB   ← Arduino 層 + 影音
```

（`.a` 很大是因為含除錯符號，linker 只挑你真正呼叫到的進去。）

> **所以 4 MB 的 `fw1` 是所有人一起分的。** 開了 WiFi + RTSP 之後，
> 留給你自己程式的空間比想像中少很多。我們的猜拳 sketch 沒開 WiFi，
> 所以很寬鬆；`AudioClassification` 範例則吃掉 84%。
...
|---|---|---|
| `fw1` | 4 MB | 4 MB |
| `fw2` | 64 KB | **4 MB**（要能裝下完整的新韌體） |
| **`nn`** | **10.56 MB** | **5.88 MB** |

**這是真實產品一定會遇到的取捨**：要能遠端更新韌體，就得留一個一樣大的備援槽，
那 4 MB 只能從模型空間扣。原廠 14 個模型加起來 37 MB，本來就塞不完；
開了 OTA 之後連 `yamnet_fp16.nb`（8.27 MB）一顆都放不下。

### 所以最後的應用長什麼樣子

```mermaid
flowchart TB
  subgraph FLASH["16 MB SPI Flash —— 斷電保留"]
    BL[boot_p / boot_s<br/>312 KB]
    FW["fw1 4 MB<br/>你的 sketch + FreeRTOS<br/>+ WiFi + BT + 影音編碼"]
    IQ[iq 768 KB<br/>ISP 調校]
    NN["nn 10.56 MB<br/>img_class_cnn.nb<br/>yolov7_tiny.nb ..."]
  end

  subgraph DDR128["128 MB DDR —— 通電才有"]
    VOEM[VOE 1 MB<br/>相機獨佔]
    GEN[一般 91 MB<br/>影像緩衝 / heap]
    NNM[NN 36 MB<br/>權重 + 中間張量]
  end
  ...
  style NN fill:#f9a825,color:#000
  style NNM fill:#f9a825,color:#000
  style NPU fill:#42a5f5,color:#000
```

**開機順序**：bootloader → 載 `fw1` → FreeRTOS 起來 → 你的 `setup()` →
`modelSelect()` 去 `nn` 分區把指定的 `.nb` 搬進 DDR 的 NN 區 → NPU 開始跑。

**一個真實產品的樣子**（例如門口攝影機）：

| 元件 | 住在哪 | 大概多大 |
|---|---|---|
| 你的應用邏輯 | `fw1` | 數十 KB ~ 數百 KB |
| WiFi + TCP/IP + MQTT | `fw1` | ~1 MB |
| H.264 編碼 + RTSP | `fw1` | ~1 MB |
| FreeRTOS + 驅動 | `fw1` | ~1 MB |
| **人臉偵測模型** | `nn` | 0.56 MB |
| **人臉辨識模型** | `nn` | 0.86 MB |
| 剩下的模型空間 | `nn` | 還很多（沒開 OTA 的話） |

> **瓶頸通常不是模型，是 `fw1` 那 4 MB。** 這跟直覺相反——
> 大家都以為 AI 板子的限制是「模型放不下」，實際上模型有 10 MB 可用，
> 反而是 WiFi + 影音那一堆函式庫把韌體空間吃光。
...
| NPU | ✓ 0.4 TOPS | ✗ | ✗ |
| 工作記憶體 | **128 MB DDR** + 512 KB SRAM | 520 KB SRAM | 192 KB SRAM |
| 模型空間 | 10.56 MB flash | 4 MB flash | 1952 KB flash |
| 相機 | ✓ 內建 | ✗ | ✗ |
...
| Pico 2 CPU | **40 ms**（實測，int8 + CMSIS-NN） |
| RT584 @48 MHz | ~126 ms（推算） |

> 差最多的其實不是速度，是**記憶體**。
> 128 MB vs 192 KB 差了 **683 倍**——這才是 AMB82 能跑 MobileNetV2、
> RT584 只能跑關鍵詞喚醒的真正原因。
...
規格書寫「0.4 TOPS」，這個數字是這樣來的。

**TOPS = Tera Operations Per Second = 每秒一兆次運算。**

```
TOPS = MAC 陣列寬度 × 2 × 時脈
                     ↑
              一次 MAC = 一次乘法 + 一次加法 = 2 個 operations
```

代 AMB82 的數字：

```
400 MAC/cycle × 2 ops/MAC × 500 MHz = 400,000,000,000 ops/s = 0.4 TOPS
```

反過來用，可以從規格書**推回硬體有多寬**：

```
硬體寬度 = 0.4e12 ÷ 2 ÷ 500e6 = 400 MAC/cycle
```

也就是說，這顆 NPU 裡大約有 **400 個乘加器同時在動**。
（真正的排列是 8×8 或 16×16 的小塊再堆疊，但對估算來說寬度就夠用。）
...
| | MNIST | 我們的猜拳模型 |
|---|---|---|
| MAC 數 | 1.34 M | 30.67 M |
| ops | 2.68 M | 61.3 M |
| **理論時間 @0.4 TOPS** | **6.7 µs** | **153 µs** |
| **實測** | **121.8 µs** | — |
| **利用率** | **5.5%** | — |

**差 18 倍。** 這不是規格書騙人，是 TOPS 本來就只是**峰值**。

### ⚠ 看到 TOPS 要問的四個問題

| 問題 | 為什麼重要 |
|---|---|
| **什麼精度？** | int8 的 TOPS 通常是 int16 的 2 倍、fp16 的 4 倍。不講精度的 TOPS 沒有意義 |
| **能餵飽嗎？** | 我們只用到 5.5%。MNIST 第一層只有 3 個輸入通道，400 個乘加器大半閒著 |
| **含不含記憶體？** | 權重搬不進來，算得再快也沒用。小模型常常是**頻寬**受限不是算力受限 |
| **是不是端到端？** | 我們端到端 ~3 ms，其中 96% 在等作業系統的 tick，跟 NPU 無關 |

**怎麼樣才餵得飽**：通道數要夠多。
第一層 3 通道 → 幾乎全閒；中間層 64 通道 → 才開始有效率。
所以**大模型的利用率反而比小模型高**——這也是為什麼拿 MNIST 量 NPU 對 NPU 不公平。
...
| **AMB82 NPU** | **0.4** |
| 手機的 NPU（近年旗艦） | 15 ~ 50 |
| 筆電的 NPU（Copilot+ 門檻） | 40+ |
| 資料中心 GPU | 1,000+ |

> **TOPS 之於 NPU，像 GHz 之於 CPU**：同一顆晶片內拿來比世代有意義，
> 跨架構比就會被騙。真正該比的是「**你的那個模型**跑一次要多久」。
